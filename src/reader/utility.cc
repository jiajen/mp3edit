...
// Generator Polynomial is 0x04c11db7
const unsigned int kCrcTable0x04c11db7[] = {
...
// it_begin starts at the lowest significant byte.
void intToBytes(unsigned int val, bool is_sync_safe,
...
Crc32::Crc32(CrcPolynomial polynomial, unsigned int initial_val,
             unsigned int final_xor) :
    _crc32(initial_val), _final_xor(final_xor) {
  switch (polynomial) {
    case CrcPolynomial::kCode0x04c11db7:
      _crc_table = kCrcTable0x04c11db7;
      break;
  }
}

void Crc32::update(const unsigned char* data, int size) {
  while (size--)
    _crc32=((_crc32<<8)&0xFFFFFF00)^_crc_table[((_crc32>>24)&0xFF)^*data++];
}

unsigned int Crc32::checksum() {
  return _crc32 ^ _final_xor;
}
